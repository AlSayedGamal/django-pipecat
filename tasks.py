...
def coverage(c):
    ...
    """
    c.run("coverage run --source django-pipecat runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/django-pipecat.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ django_pipecat")
...
def lint(c):
    ...
    """
    c.run("flake8 django-pipecat tests")
...
def release(c, bumpsize=''):
    ...
    import django_pipecat
    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=django_pipecat.__version__))
    c.run("git push --tags")
